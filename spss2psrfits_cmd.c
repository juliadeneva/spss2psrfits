...
usage(void)
{
  fprintf(stderr,"%s","   -o outfile [-g outlenGB] [-i ignored] [--] infiles ...\n");
  fprintf(stderr,"%s","      Converts search-mode WAPP data into PSRFITS\n");
  fprintf(stderr,"%s","         -o: Name of the output psrfits file\n");
  fprintf(stderr,"%s","             1 char* value\n");
  fprintf(stderr,"%s","         -g: Approx length in GB of output data files\n");
  fprintf(stderr,"%s","             1 int value between 1 and 1000\n");
  fprintf(stderr,"%s","             default: `10'\n");
  fprintf(stderr,"%s","         -i: iBOBs to ignore (no spaces)\n");
  fprintf(stderr,"%s","             1 char* value\n");
  fprintf(stderr,"%s","    infiles: Input SPSS directory name \n");
  fprintf(stderr,"%s","             1...100 values\n");
  fprintf(stderr,"%s","  version: 27Sep10\n");
  fprintf(stderr,"%s","  ");
  exit(EXIT_FAILURE);
}
/**********************************************************************/
...
parseCmdline(int argc, char **argv)
{
  ...
  for(i=1, cmd.argc=1; i<argc; i++) {
    ...
    if( 0==strcmp("-g", argv[i]) ) {
      int keep = i;
      cmd.outlenGBP = 1;
      i = getIntOpt(argc, argv, i, &cmd.outlenGB, 1);
      cmd.outlenGBC = i-keep;
      checkIntLower("-g", &cmd.outlenGB, cmd.outlenGBC, 1000);
      checkIntHigher("-g", &cmd.outlenGB, cmd.outlenGBC, 1);
      continue;
    }
    ...
    if( 0==strcmp("-i", argv[i]) ) {
      int keep = i;
      cmd.ignoredP = 1;
      i = getStringOpt(argc, argv, i, &cmd.ignored, 1);
      cmd.ignoredC = i-keep;
      continue;
    }
